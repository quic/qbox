  ...
  if (totalSize <= pagesize) {
    if (totalSize <= prp1Size) {
      ...
      if (prp1Size + prp2Size < totalSize) {
        SC_REPORT_WARNING("prplist", "Invalid DPTR size");
      }
    }
  }
...
void PRPList::processPRPListBuffer(){

  uint64_t listPRP;
  uint64_t listPRPSize;
  uint64_t currentSize = prpList.size() * pagesize;

  for (size_t i = 0; i < prpListSize; i += 8) {
    listPRP = *((uint64_t *)(buffer + i));
    listPRPSize = getPRPSize(listPRP);
    currentSize += listPRPSize;

    if (listPRP == 0) {
      SC_REPORT_WARNING("prplist", "Invalid PRP in PRP List");
    }
    ...
    // TODO(jhieb) need to test if we need a chained PRP here.
  }
}

void PRPList::getPRPListFromPRP(uint64_t base, uint64_t size) {
  // Get PRP size
  prpListSize = getPRPSize(base);
  prpListAddr = base;
  // Allocate buffer
  buffer = (uint8_t *)malloc(prpListSize);
}
...
void PRPList::read(uint64_t offset, uint64_t length, uint8_t *buffer,
  ...
  DMAFunction doRead = [this, offset, length, buffer](uint64_t, void *context) {
    ...
    for (auto &iter : prpList) {
      if (begin) {
        ...
        readContext->counter++;
        // TODO(jhieb) how to do the DMA read in qemu??
  ...
  // TODO(jhieb) how to do DMA reads in QEMU?
  ...
  DMAFunction doWrite = [this, offset, length, buffer](uint64_t,
    ...
    for (auto &iter : prpList) {
      if (begin) {
        ...
        // TODO(jhieb) how to do the DMA write in QEMU??
        /*
...
void SGL::parseSGLDescriptor(SGLDescriptor &desc) {
  switch (SGL_TYPE(desc.id)) {
    ...
    case TYPE_BIT_BUCKET_DESCRIPTOR:
      ...
      break;
    default:
      SC_REPORT_WARNING("sgl", "Invalid SGL descriptor");
      break;
  }

  if (SGL_SUBTYPE(desc.id) != SUBTYPE_ADDRESS) {
    SC_REPORT_WARNING("sgl", "Unexpected SGL subtype");
  }
}

void SGL::parseSGLSegment() {
  SGLDescriptor desc;
  DMAInitContext *pContext = (DMAInitContext *)context;
  //SGL *pThis = (SGL *)pContext->pThis;
  bool next = false;
  requiresSGLDMA = false;
    ...
    switch (SGL_TYPE(desc.id)) {
      ...
      case TYPE_LAST_SEGMENT_DESCRIPTOR:
        if (i != pContext->currentSize - 16) {
          SC_REPORT_WARNING("sgl", "Invalid SGL segment");
        }
        next = true;
  ...
  DMAFunction doWrite = [this, offset, length, buffer](uint64_t,
    ...
    for (auto &iter : chunkList) {
      if (begin) {
        ...
        if (!iter.ignore) {
          writeContext->counter++;
          // TODO(jhieb) how to do DMA writes in qemu?
          /*
          pInterface->dmaWrite(iter.addr, written,
