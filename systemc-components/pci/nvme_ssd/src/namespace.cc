...
Namespace::Namespace(ConfigData &c)
    : //pParent(p),
      pDisk(nullptr),
      cfgdata(c),
      nsid(NSID_NONE),
...
CQEntryWrapper Namespace::submitCommand(SQEntryWrapper &req, uint8_t *&buffer) {
  CQEntryWrapper resp(req);

  // TODO(jhieb) timing fixes?
    ...
    // NVM commands
      ...
      switch (req.entry.dword0.opcode) {
        ...
        case OPCODE_WRITE:
          ...
          break;
        case OPCODE_READ:
          rd_rsp = read(req, buffer);
          resp.setStatus(rd_rsp.getStatus());
          break;
        case OPCODE_COMPARE:
          compare(req);
          break;
        case OPCODE_DATASET_MANAGEMEMT:
          datasetManagement(req);
          break;
        default:
          resp.makeStatus(true, false, TYPE_GENERIC_COMMAND_STATUS,
                          STATUS_INVALID_OPCODE);
...
void Namespace::setData(uint32_t id, Information *data) {
  ...
  if (NVME_ENABLE_DISK_IMAGE) {
    uint64_t diskSize;

    std::string filename = NVME_DISK_IMAGE_PATH + std::to_string(nsid);

    if (filename.length() == 0) {
      pDisk = new MemDisk();
    }
    else if (NVME_USE_COW_DISK) {
      pDisk = new CoWDisk();
    }
    else {
      pDisk = new Disk();
    ...
    if (diskSize == 0) {
      SC_REPORT_WARNING("namespace", "Failed to open disk image");
    }
    else if (diskSize != info.size * info.lbaSize) {
      if (NVME_STRICT_DISK_SIZE) {
        SC_REPORT_WARNING("namespace", "Disk size not match");
      }
    }

    if (filename.length() > 0) {
      std::ostringstream msg;
      msg << "Using disk image at " << filename << " for NSID " << nsid;
      SC_REPORT_INFO("namespace", msg.str().c_str());
    }
  }
...
void Namespace::getLogPage(SQEntryWrapper &req) {
  CQEntryWrapper resp(req);
...
void Namespace::flush(SQEntryWrapper &req) {
  bool err = false;
...
void Namespace::write(SQEntryWrapper &req) {
  bool err = false;
  ...
  if (nlb == 0) {
    err = true;
    SC_REPORT_WARNING("namespace", "host tried to write 0 blocks");
  }
  ...
  */
  ...
  if (!err) {
    ...
    */
    ...
    if (req.useSGL) {
      pContext->dma =
          new SGL(cfgdata, empty, nullptr, req.entry.data1, req.entry.data2);
    }
    else {
      pContext->dma =
          new PRPList(cfgdata, empty, nullptr, req.entry.data1,
...
CQEntryWrapper Namespace::read(SQEntryWrapper &req, uint8_t *&buffer) {
  bool err = false;
  ...
  if (nlb == 0) {
    ...
    SC_REPORT_WARNING("namespace", "host tried to read 0 blocks");
  }
  ...
  */
  ...
  if (!err) {
      ...
      buffer = (uint8_t *)calloc(nlb, info.lbaSize);
...
void Namespace::compare(SQEntryWrapper &req) {
  bool err = false;
...
void Namespace::datasetManagement(SQEntryWrapper &req) {
  bool err = false;
  ...
  */
  ...
  if (!err) {
    ...
    if (req.useSGL) {
      ...
      pContext->dma = new PRPList(cfgdata, empty, nullptr, req.entry.data1,
