...
uint16_t CQEntryWrapper::getStatus(){
  return entry.dword3.status;
}

void CQEntryWrapper::setStatus(uint16_t status){
  entry.dword3.status = status;
}
...
void Queue::setBase(DMAInterface *p, uint64_t s, uint64_t b_addr) {
  base = p;
  stride = s;
  base_addr = b_addr;
}

uint64_t Queue::getBaseAddr(){
  return base_addr;
}
...
void CQueue::setData(CQEntry *entry) {
  if (entry) {
    // Set phase
    ...
    // Write entry
    // TODO(jhieb) support DMA writes in the queue set function?
    ...
    if (head == tail) {
      SC_REPORT_WARNING("CQueue", "Completion queue overflow");
    }
  }
}
...
void SQueue::getData(SQEntry *entry){ // , DMAFunction func, void *context) {
  if (entry && head != tail) {
    // TODO(jhieb) do i want to add this back in??
    // Read entry
