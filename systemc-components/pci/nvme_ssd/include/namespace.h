...
class Namespace {
 ...
 private:
  ...
  // TODO(jhieb) Manual config for now
  bool NVME_ENABLE_DISK_IMAGE = false;
  std::string NVME_DISK_IMAGE_PATH = "nvme.img";
  bool NVME_USE_COW_DISK = false;
  bool NVME_STRICT_DISK_SIZE = false;
  ...
  // Admin commands
  void getLogPage(SQEntryWrapper &);

  // NVM commands
  void flush(SQEntryWrapper &);
  void write(SQEntryWrapper &);
  CQEntryWrapper read(SQEntryWrapper &, uint8_t *&);
  void compare(SQEntryWrapper &);
  void datasetManagement(SQEntryWrapper &);

 public:
  Namespace(ConfigData &);
  ~Namespace();

  CQEntryWrapper submitCommand(SQEntryWrapper &, uint8_t *&);
