...
typedef struct _CQEntryWrapper {
  ...
  uint16_t getStatus(); // Used for copying status between responses.
  void setStatus(uint16_t status);  // Used for copying status between responses.
} CQEntryWrapper;
...
class Queue {
 ...
 public:
  ...
  void setBase(DMAInterface *, uint64_t, uint64_t);
};
