...
private:
    ...
    void b_transport(int id, tlm::tlm_generic_payload& trans, sc_core::sc_time& delay)
    {
        ...
        if (ti->mask_addr) trans.set_address(addr - ti->address);
        ...
        if (ti->mask_addr) trans.set_address(addr);
...
protected:
    virtual void before_end_of_elaboration()
    {
        ...
        for (auto& ti : targets) {
            ...
            m_broker.ignore_unconsumed_preset_values([ti](const std::pair<std::string, cci::cci_value>& iv) -> bool {
