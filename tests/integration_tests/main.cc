...
#define GSC_MODULE_REGISTER(__name__)                                 \
    bool __name__##_gs_module_reg_entry = gs::ModuleFactory::Reg(#__name__, (std::function<sc_core::sc_module *(sc_core::sc_module_name)>)[](sc_core::sc_module_name _instname)->sc_core::sc_module *{return new __name__(_instname);})
    
#define GSC_MODULE_REGISTER_1(__name__, __T1__)                         \
    bool __name__##_gs_module_reg_entry = gs::ModuleFactory::Reg(#__name__, (std::function<sc_core::sc_module *(sc_core::sc_module_name, __T1__)>)[](sc_core::sc_module_name _instname, __T1__ _t1)->sc_core::sc_module *{return new __name__(_instname, _t1);});
...
//                    &local_drift_before] {
                    ...
                    // care
                                         ...
                                         sc_time current=sc_core::sc_time_stamp();
                                         sc_time l_time=qk->get_current_time();
            ...
            , description("description","")
        {
            for (auto name:gs::sc_cci_children(name())) {
                ...
                if (m_broker.has_preset_value(std::string(sc_module::name())+"."+name+".moduletype")) {
                    std::string type=m_broker.get_preset_cci_value(std::string(sc_module::name())+"."+name+".moduletype").get_string();
...
public:
    tests(sc_module_name _name, Checker &_checker)
        : sc_module(_name)
        , checker(_checker)
    {
        for (auto name:gs::sc_cci_children(name())) {
...
//        std::sort (allTests.begin(), allTests.end(), [](const atest *a, const atest *b)->bool{return strcmp(a->name() , b->name());});
