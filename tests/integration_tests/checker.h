...
public:
    ...
    const char* evTypeString(eventTypes t) {
        switch (t) {
            case TimeStamp: return "Time Stamp";
            case ClockTick: return "Clock tick";
            case Budget: return "Budget";
            case TxnSent: return "Txn Sent";
            case TxnRec: return "Txn Recieved";
            case Finished: return "Model Finished";
            case StopTest: return "Stop Test";
            case StartTest: return "Start Test";
            case SetQuantum: return "n/a";
            case FinishedInstructions: return "n/a";
            default: return "unexpected event type";
        }
    }
private:
...
public:
        ...
        std::chrono::high_resolution_clock::duration push(Event &e)
	{
            ...
            if (size>=max-1) {
                ...
                delay = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - pausetime);
            }
            array[in]=e;
            in=(in+1)%max;
            size++;
...
public:
    ...
    void printevent(Event e, bool rec)
    {
                ...
                const char *timename;
                switch (e.event) {
                    case TxnSent: timename=" Delta time:";break;
                    case TxnRec: timename=" Delta time:";break;
                    case TimeStamp: timename=" Local time:";time-=startTime;break;
                    case Budget: timename=" run Budget :";break;
                    case SetQuantum: timename=" Quantum :";break;
            ...
            /* Average run skew */
            {
                ...
                for (auto run : test_p.second) {
                    for (auto evt : run) {
                        ...
                        if (evt.event == TimeStamp) {
                            sc_core::sc_time err =
                                (evt.simtime >  evt.time) ? evt.simtime-evt.time:evt.time-evt.simtime;
                            total_error+=err;
