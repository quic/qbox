...
def fastrpc_calc_test():
    ...
    parser.add_argument("-p", "--rplugconf", metavar="", help="Path to remote hexagon plugin config directory")
    ...
    if not args.img:
        ...
        return test
    if not args.rplugconf:
        print("rplugconf argument is required")
        return test

    vp_path = Path(args.exe)
    lua_path = Path(args.lua)
    img_path = Path(args.img)
    rplugconf_path = Path(args.rplugconf)
    ...
    env = {
        ...
        "QQVP_PLUGIN_DIR":rplugconf_path.as_posix(),
        "PWD": os.environ['PWD']
    }
