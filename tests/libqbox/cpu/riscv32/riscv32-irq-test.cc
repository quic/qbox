...
class CpuRiscv32TimerIrqTest : public CpuRiscvTestBench<cpu_riscv32, CpuTesterMmio>
{
    ...
    /*
     * Timeout limit for timer IRQ test - timer fires every 1ms, so allow plenty of time
     * Extended timeout for icount mode which executes much slower due to instruction counting
     * Also extended for high CPU count multithread scenarios which have synchronization overhead
     * Some multithread race conditions can take significantly longer to resolve
     */
...
public:
    ...
    void before_end_of_elaboration() override
    {
        ...
        // The timer_irq vector is still empty here, but will be initialized during timer's before_end_of_elaboration
        // We need to wait for the timer component to finish its initialization

        // Check if timer component already initialized the vector properly
        SCP_INFO(SCMOD) << "Timer IRQ vector initialized with size=" << aclint_mtimer.timer_irq.size();
    ...
    void end_of_elaboration() override
    {
        ...
        // SystemC doesn't allow port binding here, but we can manually connect the GPIO signals
        // Timer interrupts will be delivered via the QEMU device's GPIO connections
        SCP_INFO(SCMOD) << "Timer IRQ connections established via QEMU GPIO system";
    }
