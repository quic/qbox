...
uint32_t MACAddress::lo() const {
    return
            ((uint32_t) m_bytes[0] <<  0)
            | ((uint32_t) m_bytes[1] <<  8)
            | ((uint32_t) m_bytes[2] << 16)
            | ((uint32_t) m_bytes[3] << 24);
}

uint16_t MACAddress::hi() const {
    return
            ((uint16_t) m_bytes[4] << 0)
            | ((uint16_t) m_bytes[5] << 8);
}
...
bool MACAddress::set_from_str(const std::string &mac) {
    ...
    for (int i = 0; i < 6; i++) {
        std::string &s = strs[i];
        if (s.size() != 2 || s.find_first_not_of("0123456789abcdefABCDEF") != std::string::npos) { return false; }
