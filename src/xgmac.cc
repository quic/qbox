...
ssize_t Xgmac::eth_rx(const uint8_t *buf, size_t size)
{
    ...
    dma.bus_write(bd.buffer1_addr, (uint8_t *)buf, size);
...
void Xgmac::xgmac_read_desc(XGmacDesc *d, int rx)
{
    ...
    MLOG_F(SIM, TRC, "Reading XGmacDesc at addr %08" PRIx32 "\n", addr);
...
void Xgmac::xgmac_enet_send()
{
    ...
    while (1) {
        ...
        if ((bd.buffer1_size & 0xfff) > 2048) {
            MLOG_F(SIM, ERR, "qemu:%s:ERROR...ERROR...ERROR... -- "
        ...
        if (size_t(len) >= sizeof(frame)) {
            MLOG_F(SIM, ERR, "qemu:%s: buffer overflow %d read into %zu "
                    "buffer\n" , __func__, len, sizeof(frame));
            MLOG_F(SIM, ERR, "qemu:%s: buffer1.size=%d; buffer2.size=%d\n",
                    __func__, bd.buffer1_size, bd.buffer2_size);
        }

        dma.bus_read(bd.buffer1_addr, ptr, len);
        ptr += len;
        frame_size += len;
        if (bd.ctl_stat & 0x20000000) {
            /* Last buffer in frame.  */
            MLOG(SIM, TRC) << "Last buffer in frame, sending. Size: " << frame_size << "\n";
