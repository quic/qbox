...
#include "greensocs/systemc-macs/xgmac.h"
...
ssize_t Xgmac::eth_rx(const uint8_t *buf, size_t size)
{
    ...
    dma.bus_write(((bd.buffer2_addr << 32) | bd.buffer1_addr), (uint8_t *)buf, size);
...
void Xgmac::payload_recv(Payload &frame)
{
    eth_rx(frame.data(), frame.size());
}

void Xgmac::xgmac_read_desc(XGmacDesc *d, int rx)
{
    uint32_t addr = rx ? m_regs[DMA_CUR_RX_DESC_ADDR] :
        m_regs[DMA_CUR_TX_DESC_ADDR];

    SCP_TRACE(SCMOD) << "Reading XGmacDesc at addr " << std::hex << addr;
    dma.bus_read(addr, (uint8_t *)d, sizeof(*d));
}
...
void Xgmac::xgmac_write_desc(XGmacDesc *d, int rx)
{
    ...
    dma.bus_write(addr, (uint8_t *)d, sizeof(*d));
}
...
void Xgmac::xgmac_enet_send()
{
    ...
    while (1) {
        ...
        if ((bd.buffer1_size & 0xfff) > 2048) {
            SCP_ERR(SCMOD) << __func__ << ":ERROR...ERROR...ERROR... -- xgmac buffer 1 len on send > 2048 (0x" << std::hex << bd.buffer1_size << ")";
        }
        if ((bd.buffer2_size & 0xfff) != 0) {
            SCP_ERR(SCMOD) << __func__ << ":ERROR...ERROR...ERROR... -- xgmac buffer 2 len on send > 2048 (0x" << std::hex << bd.buffer2_size << ")";
        }
        ...
        dma.bus_read(((bd.buffer2_addr << 32) | bd.buffer1_addr), ptr, len);
        ptr += len;
        frame_size += len;
