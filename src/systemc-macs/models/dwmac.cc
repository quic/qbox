...
Dwmac::Dwmac(sc_module_name name)
    : sc_module(name), intr0("mac-irq"), intr1("eth-wake-irq"),
      socket("socket"), dma("dma"), phy("phy", 0x00010001, 0),
      m_tx_frame(9018u) {
  reset();
  ...
  SCP_TRACE(SCMOD) << "Mac-address initialized to " << std::hex << m_mac_0[0]
                   << ":" << std::hex << m_mac_0[1] << ":" << std::hex
                   << m_mac_0[2] << ":" << std::hex << m_mac_0[3] << ":"
                   << std::hex << m_mac_0[4] << ":" << std::hex << m_mac_0[5];
...
uint32_t Dwmac::read_word(uint32_t address) {
  ...
  case DWMAC_AN_EXPANSION:
    if (warn_anregs) {
          SCP_WARN(SCMOD) << "read to A-N registers not supported (0x"<< std::hex << (unsigned)address <<"))(will warn only once)";
      warn_anregs = false;
    } else {
...
void Dwmac::write_word(uint32_t address, uint32_t data) {
  ...
  case DMA_TX_POLL:
    ...
    if (!desc.des01.etx.own) {
      ...
      /* resume TX */
      m_dma.intr.setTxState(
          DwmacState::TX_SUSPENDED); // TODO: shoud be active state (but it is
                                     // not yet modelized)
      m_tx_event.notify();
    }
    return;
  ...
  case DMA_RX_POLL:
    ...
    if (!desc.des01.erx.own) {
      ...
      /* resume RX */
      m_dma.intr.setRxState(
          DwmacState::RX_SUSPENDED); // TODO: shoud be active state (but it is
                                     // not yet modelized)
...
bool Dwmac::tx() {
  ...
  while (!desc.des01.etx.last_segment) {
    ...
    dma.bus_read(desc.des2, m_tx_frame.data() + offset,
                 desc.des01.etx.buffer1_size);
  }
  ...
  if ((m_configuration & (1 << 10)) && desc.des01.etx.checksum_insertion) {
    ...
    if (len >= 14 && buffer[12] == 0x08 && buffer[13] == 0x00) {
      ...
      /* TCP/UDP checksum */
      if ((desc.des01.etx.checksum_insertion == 3) && len >= 24 &&
          (ip_prot == 0x06 || ip_prot == 0x11)) {
        int csum_offset;
...
bool Dwmac::rx(Payload &frame) {
  ...
  desc.des01.erx.rx_mac_addr =
      0; // extended status not available (we would need to implement it)
