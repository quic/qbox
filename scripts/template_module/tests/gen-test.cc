...
#include "greensocs/reg_model_maker/incbin.h"
    INCBIN(ZipArchive, gen_xstr(PROJECT_SOURCE) "/json.zip");
...
SC_MODULE (top) {
    SCP_LOGGER();
    gs::Router<> m_router;
    gs::json_module m_toponly;
    gs::json_module m_fulltop;
    ...
    top(sc_core::sc_module_name _name)
        ...
        , m_initiator("init")
        , m_toponly("chip",
                    zip_open_from_source(zip_source_buffer_create(gZipArchiveData, gZipArchiveSize, 0, nullptr),
                                         ZIP_RDONLY, nullptr),
                    nullptr, "", 1)
        , m_fulltop("fullchip",
                    zip_open_from_source(zip_source_buffer_create(gZipArchiveData, gZipArchiveSize, 0, nullptr),
        ...
        , m_example("example")
    {
        ...
        m_toponly.json_read_cci(std::string(m_toponly.name()),
                                zip_open(gen_xstr(PROJECT_SOURCE) "/json.zip", ZIP_RDONLY, nullptr));

        m_fulltop.json_read_cci(std::string(m_fulltop.name()),
                                zip_open(gen_xstr(PROJECT_SOURCE) "/json.zip", ZIP_RDONLY, nullptr));

        m_initiator.socket(m_router.target_socket);
        m_router.initiator_socket(m_toponly.target_socket);
        m_router.initiator_socket(m_fulltop.target_socket);
    ...
    void
    reg_mod_access(sc_core::sc_module * mod, uint64_t base)
    {
    ...
    void reg_access()
    {
        std::vector<sc_core::sc_module*> mods = objs_in<sc_core::sc_module>(gs::find_sc_obj(nullptr, m_fulltop.name()));
        for (auto m : mods) {
            wait(1, sc_core::SC_NS);
            SCP_INFO(()) << "Mod: " << m->name();
            reg_mod_access(m, 0);
