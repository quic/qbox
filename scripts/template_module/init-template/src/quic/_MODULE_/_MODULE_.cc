...
#include "quic/gen_boilerplate/incbin.h"
    ...
    INCBIN(ZipArchive__MODULE__, __FILE__ "_config.zip");

#include <quic/gen_boilerplate/reg_model_maker.h>
#include "quic/_MODULE_/_MODULE_.h"

_MODULE_::_MODULE_(sc_core::sc_module_name _name)
    : sc_core::sc_module(_name)
    , m_broker(cci::cci_get_broker())
    , m_jza(zip_open_from_source(
          zip_source_buffer_create(gZipArchive__MODULE__Data, gZipArchive__MODULE__Size, 0, nullptr), ZIP_RDONLY,
          nullptr))
    , loaded_ok(m_jza.json_read_cci(m_broker.create_broker_handle(), std::string(name()) + ".M"))
    , M("M", m_jza)
    , GSREG_CONSTRUCT(_MODULE__REGS)
    , target_socket("target_socket")
{
    SCP_TRACE(())("Constructor");
    sc_assert(loaded_ok);
    GSREG_BIND(_MODULE__REGS);
