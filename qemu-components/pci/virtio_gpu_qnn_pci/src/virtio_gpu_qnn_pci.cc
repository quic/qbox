...
#include <virtio_gpu_qnn_pci.h>

#include <libqemu/config-host.h>

virtio_gpu_qnn_pci::virtio_gpu_qnn_pci(const sc_core::sc_module_name& name, sc_core::sc_object* o,
                                       sc_core::sc_object* t)
    : virtio_gpu_qnn_pci(name, *(dynamic_cast<QemuInstance*>(o)), (dynamic_cast<qemu_gpex*>(t)))
{
}
virtio_gpu_qnn_pci::virtio_gpu_qnn_pci(const sc_core::sc_module_name& name, QemuInstance& inst, qemu_gpex* gpex)
    : QemuVirtioGpu(name, inst, "qnn-pci", gpex)
...
void virtio_gpu_qnn_pci::before_end_of_elaboration()
{
...
void module_register() { GSC_MODULE_REGISTER_C(virtio_gpu_qnn_pci, sc_core::sc_object*, sc_core::sc_object*); }
