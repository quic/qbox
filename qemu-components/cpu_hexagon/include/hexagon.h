...
public:
    ...
    qemu_cpu_hexagon(const sc_core::sc_module_name& name, sc_core::sc_object* o, uint32_t cfgbase,
                     uint32_t l2vic_base_addr, uint32_t qtimer_base_addr, uint32_t exec_start_addr, bool vp_mode = true)
        : qemu_cpu_hexagon(name, *(dynamic_cast<QemuInstance*>(o)), cfgbase, l2vic_base_addr, qtimer_base_addr,
                           exec_start_addr, vp_mode)
    {
    }
    qemu_cpu_hexagon(const sc_core::sc_module_name& name, QemuInstance& inst, uint32_t cfgbase,
                     uint32_t l2vic_base_addr, uint32_t qtimer_base_addr, uint32_t exec_start_addr, bool vp_mode = true)
    ...
    void before_end_of_elaboration() override
    {
        ...
        cpu.set_prop_int("subsystem-id", gs::cci_get<uint32_t>(m_broker, parent + ".subsystem_id"));
