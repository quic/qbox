...
apt install -y make cmake g++ wget flex bison unzip python3 python3-dev python3-pip pkg-config libpixman-1-dev libglib2.0-dev libslirp-dev libasio-dev ninja-build
...
### VNC
Qbox provides an interface which allows users to establish VNC session on to gpu outputs. <br>

One vnc session on the default gpu output
```lua
    gpu = {
...
and assign a vnc session to each output
```lua
    gpu = {
        moduletype = "virtio_gpu_gl_pci";
        outputs = 2; -- Number of virtual displays
        args = ...;
    }
    ...
    vnc0 = {
        moduletype = "vnc";
        gpu_output = 0;  -- VNC session for virtual display 1
        args = {"&platform.gpu_0"};
    }

    vnc1 = {
        moduletype = "vnc";
        gpu_output = 1; -- VNC session for virtual display 1
        args = {"&platform.gpu_0"};
    }
...
The content of the optional configuration field `params` is passed to QEMU and provide additional VNC configuration(see Qemu VNC documentation). <br>
Fields `:,head= and display=`, when passed via `params`, are not forwarded to libqemu. Those fields are generated by the `vnc` module.


By default each display is available on port `5900 + display_idex` when using tcp or `5700 + display_index` when using websocket.
QBox monitor requires vnc over websocket.
