...
UNLIKELY_TO_BE_USED = INITIAL_DDR_SPACE_14GB + OFFSET_SMEM_DDR_SPACE
--DDR_SPACE_SIZE
MINI_BOOTLOADER = UNLIKELY_TO_BE_USED
DTB_LOAD_ADDR = UNLIKELY_TO_BE_USED + 512

APSS_GIC600_GICD_APSS = 0x017a00000
...
_KERNEL64_LOAD_ADDR = INITIAL_DDR_SPACE_14GB ;--+ OFFSET_SMEM_DDR_SPACE
...
platform = {
    ...
    hexagon_start_addr = INITIAL_DDR_SPACE_14GB + 0x0B500000; -- fixme
...
-- hexagon_0 = { gdb_port = 1234 };
    ...
    ram=  {  target_socket = {address=INITIAL_DDR_SPACE_14GB, size=DDR_SPACE_SIZE}}; -- updated makena
    hexagon_ram={target_socket={address=0x0, size=0x08000000}}; -- fixme
    rom=  {  target_socket = {address=0xde000000, size=0x400 },read_only=true};
    gic=  {  dist_iface    = {address=APSS_GIC600_GICD_APSS, size=0x10000 }; -- updated makena
             redist_iface_0= {address=OFFSET_APSS_ALIAS0_GICR_CTLR, size=0xf60000}}; -- updated makena

    -- FIXME omit these for now?
    ...
    ipcc= {  socket        = {address=0x410000, size=0xfc000}}; -- same makena vs waipio/kailua

    -- FIXME: this cannot be removed, so it's configured with minimal size for now
    system={ target_socket = {address=0x30000000, size=0x10000000}};
...
--        {data=_bootloader_aarch64, address = 0x30000000};--MINI_BOOTLOADER};
--      {data=_bootloader_aarch64, address = 0};
