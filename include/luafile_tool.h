...
#define DEBUG(name, msg) std::cout<<"@"<<sc_core::sc_time_stamp()<<" /"<<(unsigned)sc_core::sc_delta_count()<<" ("<<name  <<"): "<<msg<<std::endl
...
//      lua_getfield(L, LUA_GLOBALSINDEX, "_G");
  ...
  protected:
    ...
    void parseCommandLineWithBoost(const int argc, const char* const* argv) /*throw(CommandLineException)*/ {
      DEBUG(name(), "Parse command line for --gs_luafile option ("<<argc<<" arguments)");
    ...
    void parseCommandLineWithGetOpt(const int argc, const char* const* argv) /* throw(CommandLineException) */ {
      ...
      while (1) {
        ...
        case 'p': // -p and --param
        {
            ...
            DEBUG(name(), "Setting param "<<key<<" to value "<<value);
            m_broker.set_preset_cci_value(key, cci::cci_value(cci::cci_value::from_json(value)));
            break;
        }
        case 'h': // -h and --help
        ...
        case '?':
        case ':':
          DEBUG(name(), "Option "<<c<<" not processed in lua file command line parser: "<< optopt);
          break;
        }
      }
    }
      ...
      while (lua_next(L, t) != 0) {
        ...
        case LUA_TNUMBER:
          ...
          if (strcmp(key, "math.huge") == 0 ||
            ...
            if ((uint64_t)num==num) {
                m_broker.set_preset_cci_value(key, cci::cci_value((uint64_t)num));
            } else if ((int64_t)num==num) {
                m_broker.set_preset_cci_value(key, cci::cci_value((int64_t)num));
            } else {
                m_broker.set_preset_cci_value(key, cci::cci_value(num));
            }
        ...
        case LUA_TBOOLEAN:
            m_broker.set_preset_cci_value(key, cci::cci_value((bool)lua_toboolean(L, -1)));
        ...
        case LUA_TSTRING:
          ...
          if (strcmp(key, "_VERSION") == 0 ||
            ...
            if (GC_LUA_DEBUG) fprintf(stderr, "(%s) %s   (ignored because it's Lua specific)\n", lua_typename(L, lua_type(L, -1)), key);
              ...
              m_broker.set_preset_cci_value(key, cci::cci_value(std::string(lua_tostring(L, -1))));
