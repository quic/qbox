 ...
 *  Copyright (c) 2015-2020 Clement Deschamps and Luc Michel
 *
...
#include "libqbox/utils/mii.h"
#include "libqbox/models/dma.h"
#include "libqbox/models/phy.h"
...
public:
    ...
    void clear(uint32_t clear) {
        ...
        if ((clear & NIS_mask) && !(clear & NIS_bit)) {
            std::cout << "Clearing normal interrupts without clearing NIS\n";
        }

        if ((clear & AIS_mask) && !(clear & AIS_bit)) {
            std::cout << "Clearing abnormal interrupts without clearing AIS\n";
        }

        m_interrupts &= ((~clear) & IS_mask);

        if ((m_interrupts & NIS_mask) && (m_regmask & NIS_bit))
            m_interrupts |= NIS_bit;
        if ((m_interrupts & AIS_mask) && (m_regmask & AIS_bit))
            m_interrupts |= AIS_bit;
    ...
    void set_interrupts(uint32_t mask) {
        m_regmask = (mask & (IS_mask | IS_bits));
    }

    void set_status(uint32_t value) {
        clear(value & (IS_mask | IS_bits));
    }
...
public:
    sc_core::sc_signal<bool, sc_core::SC_MANY_WRITERS> intr0, intr1;
    ...
    {
        ((Dwmac *)opaque)->payload_recv(frame);
    }
