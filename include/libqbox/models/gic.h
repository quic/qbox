...
class QemuArmGicV3 : public QemuComponent {
public:
    sc_core::sc_vector<QemuInPort> spis;
...
public:
    QemuArmGicV3(sc_core::sc_module_name name)
        : QemuComponent(name, "arm-gicv3")
        , spis("spi")
    {
        m_num_irq = 256;
        m_revision = 3;
    ...
    void set_qemu_properties_callback()
    {
        ...
        m_obj.set_prop_int("num-cpu", m_cores.size());
        m_obj.set_prop_int("num-irq", m_num_irq + GIC_INTERNAL);
        m_obj.set_prop_int("len-redist-region-count", 1);
        m_obj.set_prop_int("redist-region-count[0]", m_cores.size());
        m_obj.set_prop_bool("has-security-extensions", false);
    }
    ...
    void end_of_elaboration()
    {
        ...
        for (auto &core : m_cores) {
            ...
            mr = m_lib->object_new<qemu::MemoryRegion>();
            mr.init_alias(cpu, "cpu-alias", cpu_mr, 0, cpu_mr.get_size());
            core->m_ases[0].mr.add_subregion(mr, 0xc8010000);
        }
#endif
