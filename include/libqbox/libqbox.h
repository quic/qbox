...
public:
    ...
    {
        m_obj.set_prop_int("num-cpu", m_num_cpu);
    ...
    void end_of_elaboration()
    {
        ...
        qemu::MemoryRegion dist_mr = sbd.mmio_get_region(0);
        mr = m_lib->object_new<qemu::MemoryRegion>();
        mr.init_alias(cpu, "dist-alias", dist_mr, 0, dist_mr.get_size());
        m_cpu->m_ases[0].mr.add_subregion(mr, 0xc8000000);

        qemu::MemoryRegion cpu_mr = sbd.mmio_get_region(1);
        mr = m_lib->object_new<qemu::MemoryRegion>();
        mr.init_alias(cpu, "cpu-alias", cpu_mr, 0, cpu_mr.get_size());
        m_cpu->m_ases[0].mr.add_subregion(mr, 0xc8001000);

        qemu::Gpio cpu_irq = cpu.get_gpio_in(0);
        sbd.connect_gpio_out(0, cpu_irq);
        qemu::Gpio cpu_fiq = cpu.get_gpio_in(1);
        sbd.connect_gpio_out(1, cpu_fiq);

        int ppibase = m_num_irq + 0 * GIC_INTERNAL + GIC_NR_SGIS;
...
#define ARRAY_SIZE(x) ((sizeof(x) / sizeof((x)[0])))
        ...
        for (size_t irq = 0; irq < ARRAY_SIZE(timer_irq); irq++) {
            qemu::Device gicdev = qemu::Device(get_qemu_obj());
            qemu::Gpio gpio = gicdev.get_gpio_in(ppibase + timer_irq[irq]);
            cpu.connect_gpio_out(irq, gpio);
        }
    }
};
