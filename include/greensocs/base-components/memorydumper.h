        ...
        for (std::string m : gs::find_object_of_type<Memory<BUSWIDTH>>()) {
            ...
            for (uint64_t offset = 0; offset < size;) {
                ...
                if (!initiator_socket->get_direct_mem_ptr(trans, dmi)) {
                    ...
                    info << name() << " : Error loading data (no DMI) from memory @ "
                         << "0x" << std::hex << addr + offset;
                    SC_REPORT_WARNING("Loader", info.str().c_str());
                    break;
                }
                uint64_t size = (dmi.get_end_address() -
                ...
                if (fwrite(ptr, size, 1, out) != 1) {
                    std::stringstream info;
                    info << name() << " : Error saving data to file " << fname;
                    SC_REPORT_WARNING("Loader", info.str().c_str());
                }
                offset += size;
            }
...
public:
    tlm_utils::simple_initiator_socket<MemoryDumper<BUSWIDTH>>
        initiator_socket;
