...
#define BROKERNAME "gs::ConfigurableBroker"
  ConfigurableBroker(const std::string &name = BROKERNAME,
      ...
      : m_orig_name(hierarchical_name()),
        ...
        conf_file("lua_conf", "",
                  ...
                  "Lua configuration file", CCI_RELATIVE_NAME,
                  get_cci_originator(name.c_str())),
   ...
   */
  ConfigurableBroker(const int argc, const char *const *argv,
  ...
  get_param_handles(const cci_originator &originator) const {
    if (has_parent) {
      std::vector<cci_param_untyped_handle> p_param_handles =
          m_parent.get_param_handles();
      std::vector<cci_param_untyped_handle> param_handles =
          consuming_broker::get_param_handles(originator);
