...
class SigHandler{
    ...
    inline void add_to_block_set(int signum){
        sigaddset(&m_sigs_to_block, signum);
    }

    inline void reset_block_set(){
        sigemptyset(&m_sigs_to_block);
    }

    inline void block_signal_set(){
        sigprocmask( SIG_BLOCK, &m_sigs_to_block, NULL);
    }
    ...
    */
    ...
    inline int get_write_sock_end() const{
        return self_sockpair_fd[1];
    }

    inline  void set_sig_num(int val){
        sig_num = val;
    }
        ...
        pass_handler = std::thread([this](){
            ...
            while(!stop_running){
                ...
                if((ret == -1 && errno == EINTR) || ret == 0 /*timeout*/)
                    continue;
                else if(self_pipe_monitor.revents & (POLLHUP | POLLERR | POLLNVAL | POLLRDHUP )){
                    return;
                }     
    ...
    SigHandler():
        m_signals{
            {SIGINT,  Handler_CB::EXIT},
            {SIGTERM, Handler_CB::EXIT},
            {SIGQUIT, Handler_CB::EXIT},
            {SIGSEGV, Handler_CB::EXIT},
            {SIGABRT, Handler_CB::EXIT},
            {SIGBUS,  Handler_CB::EXIT}},
    ...
    inline void _update_sig_cb(int signum, Handler_CB s_cb){
        switch(s_cb){
            case Handler_CB::EXIT:
                signal(signum, SigHandler::exit_sig_handler);
                break;
            case Handler_CB::PASS:
                signal(signum, SigHandler::pass_sig_handler);
                break;
            case Handler_CB::IGN:
...
class ProcAliveHandler{
    ...
    void wait_child(pid_t cpid){
        child_waiter = std::thread([](pid_t cpid){
            pid_t m_wpid;
            int wstatus;
            for(;;){
    ...
    inline void check_parent_conn_sth(std::function<void()> on_parent_exit){
        ...
        for(;;){
            ...
            if(ret == -1 && errno == EINTR)
                continue;
            else if(parent_connected_monitor.revents & (POLLHUP | POLLERR | POLLNVAL | POLLRDHUP )){
                on_parent_exit();
                exit(EXIT_SUCCESS);
            }
            else{
